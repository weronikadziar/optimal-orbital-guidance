function ocp = ocp_translation_2D_swarm(params)
...
nc = nchoosek(na,2);
constr_expr_h = SX.sym('pos_rel', [nc+na 1]); % avoid bumping into other agents
constr_lh = zeros(nc+na,1);
constr_uh = repmat(200,nc+na,1);

i = 1;
for agent = 1:na
    pos_agent = sym_x(1+(agent-1)*ndim*2:2+(agent-1)*ndim*2);
    constr_expr_h(i) = norm(pos_agent);
    constr_lh(i) = params.min_to_target;
    i = i + 1;
    for neig = agent+1:na
        pos_neig = sym_x(1+(neig-1)*ndim*2:2+(neig-1)*ndim*2);
        constr_expr_h(i) = norm(pos_neig - pos_agent);
        constr_lh(i) = params.min_btw_agents;
        i = i + 1;
    end
end
